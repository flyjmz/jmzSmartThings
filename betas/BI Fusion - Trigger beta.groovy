...
Version 3.0.2 - 30Oct2017	Added triggers for: acceleration, presence, shock, smoke, carbonMonoxide, and water sensors.
...
def mainPage() {
  return dynamicPage(name: "mainPage", title: "BI Fusion Custom Camera Triggers", submitOnChange: true) {
      ...
      section("Move Camera to Preset") {
          paragraph "Do you want to also move your camera to a preset position?"
          ...
          if (usePreset) {
              if (usingCameraDTH) {
                  ...
                  input "biPreset", "number", title: "Preset#", required: true
              }
          }
      }
  ...
  if (usingCameraDTH) {
      ...
      for (cameraDevice in biCamerasSelected) {
          if (usePreset) {
              ...
              def presetString = 7 + settings[presetInput].toInteger()  //todo - if preset is 1, do I need to make this "7+1" or "8".  Also, it says unauthorized for ptz if I don't include the user&pass, but when I do the returned body is  null and I don't get a reaction.
              presetCommand = "/cam/${cameraDevice.name}?pos=${presetString}&user=${parent.username}&pw=${parent.password}" //todo - find out what the parse message that comes back looks like
              log.info "Moving ${cameraDevice.displayName} to preset ${settings[presetInput]}"
              talkToHub(presetCommand)
          }
          triggerCommand = "/admin?camera=${cameraDevice.name}&trigger&user=${parent.username}&pw=${parent.password}"
          talkToHub(triggerCommand)
      }
  } else {
      if (usePreset) {
          def presetString = 7 + biPreset.toInteger()  //todo update
          presetCommand = "/cam/${biCamera}/pos=${presetString}" //todo update   //todo - find out what the parse message that comes back looks like
          log.info "Moving ${biCamera} to preset ${biPreset}"
          talkToHub(presetCommand)
      }
      log.info "Triggering ${biCamera}"
      triggerCommand = "/admin?camera=${biCamera}&trigger&user=${parent.username}&pw=${parent.password}"
      talkToHub(triggerCommand)
  }
}

def talkToHub(commandPath) {
                                  ...
                                  for (cameraDevice in biCamerasSelected) {
                                      ...
                                      httpPostJson(uri: parent.host + ':' + parent.port, path: '/json',  body: ["cmd":"trigger","camera":shortName,"session":session]) { response4 ->
                                          ...
                                          if (response4.data.result == "success") {
                                              ...
                                              if (!receiveAlerts) sendNotificationEvent("Blue Iris Fusion triggered camera '${shortName}'")
                                              if (receiveAlerts) parent.send("Blue Iris Fusion triggered camera '${shortName}'")
                                          } else {
